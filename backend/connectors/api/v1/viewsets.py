...
class NewConnectorsp657ViewSet(BaseConnectorViewSet):
    PASSWORD = settings.CONNECTOR_NEW_CONNECTORSP657_PASSWORD
    USERNAME = settings.CONNECTOR_NEW_CONNECTORSP657_USERNAME
    BASE_URL = "https://api.golmal.com"
    AUTH_TYPE = "basic"
    IDENTIFIER = "CONNECTOR_NEW_CONNECTORSP657"

    @action(detail=False, methods=["get"], url_path="testing/golmal")
    def testing_golmal(self, request, *args, **kwargs):
        params = request.query_params
        payload = request.data
        data = self._api_call(
            "/testing/golmal", request_type="get", payload=payload, params=params
        )
